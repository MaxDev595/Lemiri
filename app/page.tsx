...
const copy = {
  ru: {
    ...
    eyebrow: "AI-сотрудник уже на связи",
    ...
    how: {
      ...
      steps: [
        ...
        ["03", "Подключите канал", "Сайт, WhatsApp, Telegram, CRM или календарь — всё собирается в один процесс."],
        ["04", "Проверьте и запустите", "Протестируйте диалоги и включите AI-сотрудника для клиентов."],
      ],
    },
    roles: {
    ...
    faq: {
      ...
      items: [
        ...
        ["Что происходит, когда нужен человек?", "AI-сотрудник передаёт диалог по заданным правилам и добавляет резюме, чтобы менеджер быстро продолжил разговор."],
      ],
    },
    cta: ["Ваш следующий сотрудник уже готов", "Пусть клиенты получают", "ответ, пока вы заняты бизнесом", "Создайте первого AI-сотрудника и проведите тестовый диалог.", "Начать бесплатно", "Запросить демо"],
    footer: ["Готовый цифровой сотрудник для вашего бизнеса.", "Документация", "© 2026 Lemiri AI"],
    mock: ["Новая заявка", "квалифицирована", "Анна К.", "Хочет записаться на консультацию", "Диалоги сегодня", "Ответить клиенту..."],
  },
  en: {
    ...
    how: {
      ...
      steps: [
        ...
        ["03", "Connect a channel", "Website, WhatsApp, Telegram, CRM, or calendar — all in one flow."],
    ...
    mock: ["New lead", "qualified", "Anna K.", "Wants to book a consultation", "Conversations today", "Write a reply..."],
  },
} as const;
  ...
  return (
    <main>
      ...
      <section className="hero shell" id="top">
        ...
        <div className="artifact leadCard"><div className="artifactTop"><span>{t.mock[0]}</span><span className="live">● {t.mock[1]}</span></div><strong>{t.mock[2]}</strong><small>{t.mock[3]}</small><div className="leadMeta"><span>CRM<br /><b>Lead</b></span><span>Calendar<br /><b>16:00</b></span></div></div>
        ...
        <div className="artifact chatCard"><div className="avatar">AI</div><div><b>Lemiri</b><small>{lang === "ru" ? "Отвечает сейчас" : "Replying now"}</small></div><p>{lang === "ru" ? "На завтра есть два свободных окна: 12:30 и 16:00. Какое вам удобнее?" : "Tomorrow has two available slots: 12:30 and 16:00. Which one works better?"}</p><div className="reply">{t.mock[5]} <span>↑</span></div></div>
        <div className="artifact handoffCard"><small>{lang === "ru" ? "Передача менеджеру" : "Human handoff"}</small><b>{lang === "ru" ? "Клиент готов обсудить детали" : "Customer is ready to discuss details"}</b><span>{lang === "ru" ? "Резюме подготовлено" : "Summary prepared"} →</span></div>
      </section>
      ...
      <section className="section shell" id="capabilities">
        ...
        <div className="featureGrid">{t.capabilities.cards.map(([tag, title, text], index) => <article className={`feature ${index === 0 ? "large" : ""} ${index === 2 ? "peach" : ""} ${index === 3 ? "wide" : ""}`} key={title}><span className="tag">{tag}</span><h3>{title}</h3><p>{text}</p>{index === 0 && <div className="miniChat"><span>Customer</span><p>Do you deliver today?</p><span>Lemiri · source: FAQ</span><p>Yes, orders placed before 5 PM can be delivered today.</p></div>}{index === 1 && <div className="sourceRows"><span>✓ Website</span><span>✓ Services.pdf</span><span>✓ FAQ</span></div>}{index === 3 && <div className="actionFlow"><span>Chat completed</span><b>→</b><span>Lead created</span><b>→</b><span>Team notified</span></div>}</article>)}</div>
      </section>
      ...
      <section className="section shell proof"><div className="proofText"><span className="kicker">{t.proof.kicker}</span><h2>{t.proof.title[0]} <em>{t.proof.title[1]}</em></h2><p>{t.proof.text}</p><ul>{t.proof.list.map((item) => <li key={item}>{item}</li>)}</ul></div><div className="dashboard"><div className="dashTop"><b>{lang === "ru" ? "Обзор недели" : "Weekly overview"}</b><span>1–7 Aug</span></div><div className="metrics"><span><small>Messages</small><b>846</b><i>↑ 18%</i></span><span><small>Leads</small><b>127</b><i>↑ 31%</i></span><span><small>Bookings</small><b>64</b><i>↑ 12%</i></span></div><div className="chart"><i/><i/><i/><i/><i/><i/><i/><i/><i/><i/><i/><i/></div><div className="dashFoot">{["Mon","Tue","Wed","Thu","Fri","Sat","Sun"].map((day) => <span key={day}>{day}</span>)}</div></div></section>
